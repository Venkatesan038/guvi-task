...
if (isset($_SESSION['id']) && isset($_SESSION['fname']) && isset($_SESSION['dob'])&& isset($_SESSION['age'])&& isset($_SESSION['cno'])) {
      ...
      if (isset($_FILES['pp']['name']) AND !empty($_FILES['pp']['name'])) {
         ...
         if($error === 0){
            ...
            if(in_array($img_ex_to_lc, $allowed_exs)){
               ...
               // update the Database
               $sql = "UPDATE users 
                       SET fname=?, dob=?, age=?, cno=?, username=?, pp=?
                       WHERE id=?";
               $stmt = $conn->prepare($sql);
               $stmt->execute([$fname, $dob, $age, $cno, $uname, $new_img_name, $id]);
       	...
       	$sql = "UPDATE users 
       	        SET fname=?, dob=?, age=?, cno=?, username=?
                WHERE id=?";
       	$stmt = $conn->prepare($sql);
       	$stmt->execute([$fname, $dob, $age, $cno, $uname, $id]);
